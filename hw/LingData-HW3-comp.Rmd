...
Посмотрим на значения в интервалах `[5:10]` и дальше: их 5 штук.
Остальных значений 15. Всего 20. Значит, 5/20 = четверть.

Ответ: четверть.

b. Indicate the interval where the median of this sample can lie. Explain your answer.

Вычтем из двадцати десять, получим десять. Значит, медиана будет
примерно где-то на десятом значении, то есть в интервале `[-5:10]`.

Ответ: `[-5:10]`

c. How the histogram will change if we add an element $7$ to the sample? Explain your answer.

Интервал `[5:10]` вырастет на один пункт.
...
На второй, так как на глаз там будут больше средние отклонения.
...
Скорее на первой, по той же причине.
...
Кажется, что большинство наоборот сделали меньше 35-ти ошибок.
...
$H_0$: распространённость диалектной формы в Городе не отличается от распространённости в стране
...
Так как R в этой части использовать нельзя, используем Python
```
>>> from scipy.stats import chi2_contingency
>>> country_city = [[20, 40], [9, 51]]
>>> chi2_contingency(country_city)[1]
0.032973068345691796
```

#### 4.3 Conclusion
Will you reject null hypothesis? Use significance level of 5%.

Да, p-value меньше пяти сотых.
...
Да (только информантов мало, к тому же хотелось бы видеть выборку для страны в целом).
...
```{r}
table(words) * 100 / sum(table(words))
```
